...
int merkle_create(trit_t *const tree, size_t const base_size,
  ...
  size_t leaf_index;
  for (leaf_index = 0; leaf_index < base_size; leaf_index++) {
  ...
  // hash tree
  size_t cur_size, width, depth;
  for (cur_size = base_size, depth = td; depth > 0; depth--) {
    for (width = 0; width < cur_size; width += 2) {
...
int merkle_branch(trit_t const *const tree, trit_t *const siblings,
  ...
  size_t i, depth_index;
  for (i = 0, depth_index = tree_depth - 1; depth_index > 0;
       i++, depth_index--) {
    if (sibling_index & 1) {
      sibling_index--;
    } else {
...
void merkle_root(trit_t *const hash, trit_t const *const siblings,
                 ...
                 Curl *const c) {
  size_t i, j;
  for (i = 0, j = 1; i < siblings_number; i++, j <<= 1) {
